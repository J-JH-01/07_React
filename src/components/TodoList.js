...
// 리액트 공부 하는 사이트임
// mutates 라는건 원본을 건드는거라 쓰면 위험하다는 것들임
...
const TodoList = () => {
  ...
  // Add Todo 버튼 클릭시 todoList 상태에 업데이트 이벤트 핸들러
  const handleAddTodo = () => {
    // javascript spread 연산자 (...) : 기존배열이나, 객체의 전체 또는 일부를 다른 배열이나 객체로 복사함
    // 참조 타입(배열이나 객체)인 상태(state)는 불변성을 유지해야하므로, 기존 배열을 직접 수정하지 않고 새로운 배열을 생성해야함
    // 불변성을 유지해야 하는 이유 : React가 상태 변경을 감지해야 하기 때문에
  ...
  const handleDeleteTodo = (index) => {
    // splice 함수 : mutates 함수라서 원본이 변경되는 함수이므로 state인 todoList에 직접적 사용 불가(불변성의 법칙)
      ...
      <ul>
        {
          todoList.map( (todo, index) => (
              <li key={index}>
                ...
                <button onClick={ () => handleToggleTodo(index) }>{todo.isDone ? '완료' : '미완료'}</button>
