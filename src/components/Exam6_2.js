...
// Context API를 사용 했을 때

// Context API : React 컴포넌트 트리 전체에서 데이터를 공유할 수 있는 방법을 제공
//               중첩된 구조에서 데이터를 전달하는데 있어서 불필요한 props drilling (상태 내리꽃기)를 방지함
//               사용하기 위해선 useContext hook이 필요함

// createContext : Context 객체를 생성 시 사용\
// Context : react에서 컴포넌트 계층 구조를 통해 데이터를 효율적으로 전달하기 위한 매커니즘(방식,작동원리)
...
const Exam6_2 = () => {
  ...
  // UserContext.Provider : 하위 컴포넌트에게 데이터를 전달할 때 사용
  return (
    <UserContext.Provider value={user}>
      <h1>부모에서부터 전달되는 {user}</h1>
      <Child1 />
      {/* props로 전달하지 않아도 과연 가능할까? */}
...
const Child2 = () => {
  ...
  return (
    <>
    <h1>Child 2 중간에 한번 찍어보기 : {user}</h1>
    <Child3 />
    </>
  )
}
